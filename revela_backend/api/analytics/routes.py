...
def _get_all_analytics_inner(F=None):
    ...
    cur.execute(
        "SELECT COUNT(*) AS n FROM geospatial_logs g WHERE g.flagColor != 'Green' AND NOT EXISTS (SELECT 1 FROM official_registry o WHERE LOWER(TRIM(o.businessName)) = LOWER(TRIM(g.detectedName)) AND o.barangayID = g.barangayID)" + geo_g,
        geo_g_p,
    )
    ...
    cur.execute(f"""
        ...
        LEFT JOIN geospatial_logs g ON g.barangayID = b.barangayID 
            AND NOT EXISTS (
                SELECT 1 FROM official_registry o 
                WHERE LOWER(TRIM(o.businessName)) = LOWER(TRIM(g.detectedName)) 
                  AND o.barangayID = g.barangayID
            ){geo_on_g}
        WHERE 1=1 {brgy_b}
    ...
    cur.execute(f"""
        ...
        WHERE 1=1 AND g.flagColor != 'Green' 
          AND NOT EXISTS (
              SELECT 1 FROM official_registry o2 
              WHERE LOWER(TRIM(o2.businessName)) = LOWER(TRIM(g.detectedName)) 
                AND o2.barangayID = g.barangayID
          ) {geo_g}
        GROUP BY category
        ORDER BY flagged_count DESC
        LIMIT 10
